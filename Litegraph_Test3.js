...
function _CustomGRANode(){return(
  class CustomGRANode {
    constructor(){
      this.title = "GRA";
      this.addInput("A","number");
...
function _graph(graphCell,LiteGraph,CustomMultNode,ObservableNode, CustomGRANode, $0)
{
  ...
  LiteGraph.registerNodeType("custom/GRA", CustomGRANode);
  ...
  var nodeAdd = LiteGraph.createNode("custom/GRA");
  nodeAdd.pos = [500,350];
  nodeAdd.title = "Addieren";
  nodeAdd.resizable = false;
  nodeAdd.horizontal = true;
  nodeAdd.flags.allow_interaction = false;
  graph.add(nodeAdd);

  // var nodeMult2 = LiteGraph.createNode("custom/multiply");
  // nodeMult2.pos = [500,350];
  // graph.add(nodeMult2);
  ...
  nodeConstA.connect(0, nodeAdd, 0)
  nodeConstB.connect(0, nodeAdd, 1)

  nodeAdd.connect(0, nodeWatch2, 0)
...
export default function define(runtime, observer) {
  ...
  main.variable(observer("CustomGRANode")).define("CustomGRANode", _CustomGRANode);
  main.variable(observer("ObservableNode")).define("ObservableNode", _ObservableNode);
  main.variable(observer("graph")).define("graph", ["graphCell","LiteGraph","CustomMultNode","ObservableNode", "CustomGRANode", "mutable results"], _graph);
